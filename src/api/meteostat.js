...
class MeteostatAPI {
   ...
   */
  async fetchHistorical(latitude, longitude, startDate, endDate, apiKey = null) {
    try {
      ...
      while (attempt < maxAttempts) {
        try {
          response = await safeApiFetch(url, {
            method: 'POST',
            headers: headers,
          ...
          const isClientError = /HTTP Fehler 4\d\d|401|403|404|429|API key|Invalid/.test(msg);
          ...
          console.warn(`Meteostat Versuch ${attempt} fehlgeschlagen, erneut in ${waitMs}ms...`);
        }
      }
  ...
  _validateDates(startDate, endDate) {
    if (!startDate || typeof startDate !== 'string') {
      return { valid: false, error: 'Startdatum erforderlich (Format: YYYY-MM-DD)' };
    }

    if (!endDate || typeof endDate !== 'string') {
      return { valid: false, error: 'Enddatum erforderlich (Format: YYYY-MM-DD)' };
    }

    // Basic format validation
    const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
    if (!dateRegex.test(startDate) || !dateRegex.test(endDate)) {
      return { valid: false, error: 'Ungültiges Datumformat (erwartet: YYYY-MM-DD)' };
    }
    ...
    if (diffDays > 365) {
      return { valid: false, error: 'Zeitraum darf 365 Tage nicht überschreiten' };
    }
  ...
  _validateResponse(data) {
    ...
    if (!Array.isArray(data.data)) {
      return { valid: false, error: 'Ungültige Meteostat Antwort: Keine Datenarray' };
    }

    if (data.data.length === 0) {
      return { valid: false, error: 'Meteostat: Keine Daten für den angegebenen Zeitraum' };
    }
  ...
  formatForDisplay(data) {
    ...
    return {
      daily: data.daily.map(d => ({
        ...
        precipitation: d.precipitation !== null ? `${d.precipitation.toFixed(1)} mm` : 'N/A',
        wind_speed: d.wind_speed !== null ? `${d.wind_speed.toFixed(1)} m/s` : 'N/A',
